...
fn (mut gen Gen) for_in_stmt(node ast.ForInStmt) string {
	...
	if node.high.type_name() == 'v.ast.EmptyExpr' { // in an array
		...
		if !gen.file.scope.known_var(temp_name) {	//if doesn't exits, create it
			mut temp_var := ast.Var {
				name: temp_name
				typ: ast.Type(ast.Kind.int)
			}
			gen.file.scope.objects[temp_name] = temp_var //
			gen.defs += 'int ${temp_name};\n'
		}
		var := *gen.file.scope.children[0].find_var(node.cond.str()) or { }
